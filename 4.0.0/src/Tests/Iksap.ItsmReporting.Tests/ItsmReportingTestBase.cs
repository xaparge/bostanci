...
{
    public abstract class ItsmReportingTestBase : AbpIntegratedTestBase<ItsmReportingTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ItsmReportingTestBase()
        {
        ...
        protected void UsingDbContext(Action<ItsmReportingDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ItsmReportingDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ItsmReportingDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ItsmReportingDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ItsmReportingDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ItsmReportingDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ItsmReportingDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ItsmReportingDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ItsmReportingDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ItsmReportingDbContext, Task<T>> func)
        {
            T result;
